...
def sum_grp[T](ts: TimeseriesC[List[T]], f: T => Int) = ts.map(_.map(f).sum)

def ts_sum[T : Numeric](ts: TimeseriesC[T]) =
  (for(x <- ts.from to ts.to) yield ts.fn(x)).sum

def smoothe[T : Numeric](ts: TimeseriesC[T], window_size: Int) =
...
def forecast[T: Numeric](ts: TimeseriesC[T]) : TimeseriesC[Double] = {
  val sts  = smoothe(ts, 3);
  val avg : Double = sts(ts.to - 1);

  new TimeseriesC(ts.to, (1.0/0).toInt, _ => avg)
}

def rms_volatility(ts: TimeseriesC[Double]) : Double = {
  var sm = 0.0;
...
def main_periodicity[T: Numeric](ts: TimeseriesC[T], max_period: Int) : Int = {
...
def super_forecast[T: Numeric](ts: TimeseriesC[T]) =
...
class TimeseriesC[T](_from: Int, _to: Int, _fn: Int => T) {
  def from = _from
  def to = _to
  def fn = _fn
  ...
  def map[U](f: T => U) = new TimeseriesC[U](_from, _to, (t: Int) => f(_fn(t)))

  def apply(t: Int) = _fn(t)

  def window(window_size: Int) : TimeseriesC[IndexedSeq[T]] = {
    assert(window_size >= 1);
    assert(_to - _from + 1 >= window_size);

    def w(t: Int) = for(i <- 1 to window_size) yield _fn(t - window_size + i)

    new TimeseriesC(_from + window_size - 1, _to, w)
  }

  def start_from(start: Int) = {
    assert(start >= _from);
    new TimeseriesC(start, _to, _fn)
  }

  def end_at(end: Int) = {
    assert(end <= _to);
    new TimeseriesC(_from, end, _fn)
  }
